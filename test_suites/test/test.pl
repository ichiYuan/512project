verify(id4,content,name).
verify(id4,content,'your name').
verify('id11','color',white).
verify('id11','color',black).
verify('id8','font-family',georgia).
verify('id8','font-family',arial).
verify('logo','font-family',arial).
verify('logo','font-family',georgia).
verify(id13,href,A).
verify(id13,href,'#/about.html').
img_verify(id9,left_pad,166).
img_verify(id9,left_pad,163).
img_verify(id9,pos,right).
img_verify(id9,pos,left).
img_verify(id9,aspect_ratio,A).
